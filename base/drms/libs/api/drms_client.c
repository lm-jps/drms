/*
 *  drms_client.c						2007.11.20
 *
 *  functions defined:
 *	drms_connect
 *	drms_send_commandcode
 *	drms_send_commandcode_noecho
 *	drms_connect_direct
 *	drms_disconnect
 *	drms_disconnect_now
 *	drms_commmit
 ...
 *	drms_sequence_drop
 *	drms_sequence_create
 *	drms_sequence_getnext
 ...
 *
 */
DRMS_Session_t *drms_connect (char *host, unsigned short port, char *username,
    char *passwd) {
  int i;
...
DRMS_Session_t *drms_connect_direct (char *dbhost, char *dbuser,
    char *dbpasswd, char *dbname, char *sessionns) {
  DRMS_Session_t *session;
  ...
  if (sessionns) {
    ...
    sprintf (query,
        "select c.relname from pg_class c, pg_namespace n where n.oid = c.relnamespace and n.nspname='admin' and c.relname='sessionns'");
    tresult = db_query_txt (session->db_handle, query);
    if (tresult->num_rows) {
      db_free_text_result (tresult);    
      sprintf (query,
          "select sessionns from admin.sessionns where username='%s'",
...
#ifndef DRMS_CLIENT
  if (session->db_direct) {
    return db_query_bin_array (session->db_handle,query, n_args, intype, argin);
  } else
#else
    XASSERT(session->db_direct==0);
#endif
...
int drms_dmsv (DRMS_Session_t *session, int *row_count,  char *query,
  ...
  status = drms_dms_array (session, row_count, query, n_rows, n, intype, argin);
 failure:
  va_end (ap);
  return status;
}
 ...
 *
 */
int drms_dms_array (DRMS_Session_t *session, int *row_count, char *query,
    int n_rows, int n_args, DB_Type_t *intype, void **argin) {  
#ifndef DRMS_CLIENT
  if (session->db_direct) {
    return db_dms_array (session->db_handle, row_count, query, n_rows, n_args,
        intype, argin);
  } else
#else
    XASSERT(session->db_direct==0);
#endif
  {
    ...
    return db_client_dms_array (session->sockfd, row_count, query, n_rows,
        n_args, intype, argin);
  }
}
/*
 ...
 *    list of void * and calls db_dms_array.
 */
int drms_bulk_insertv (DRMS_Session_t *session, char *table, int n_rows,
    int n_cols, ...) {
  int status=1;
  DB_Type_t intype[MAXARG];
  void *argin[MAXARG];
  ...
  status = drms_bulk_insert_array (session, table, n_rows, n_cols, intype,
      argin );
  va_end (ap);
  return status;
}
 ...
 *
 */
int drms_bulk_insert_array (DRMS_Session_t *session, char *table, int n_rows,
    int n_args, DB_Type_t *intype, void **argin) {  
#ifndef DRMS_CLIENT
  if (session->db_direct) {
    return db_bulk_insert_array (session->db_handle, table, n_rows, n_args,
        intype, argin);
  } else
#else
    XASSERT(session->db_direct==0);
#endif
  {
    ...
    return db_client_bulk_insert_array (session->sockfd, table, n_rows, n_args,
        intype, argin );
  }
}
/*
 ...
 *
 */
long long *drms_alloc_recnum (DRMS_Session_t *session, char *series, DRMS_RecLifetime_t lifetime, int n)
{
  int i, status;
  long long *seqnums;
  struct iovec vec[4];
...
int drms_newslots (DRMS_Env_t *env, int n, char *series, long long *recnum,
...
#endif
  {
	...
	/*  We didn't find the storage unit in the cache. Insert a new entry  */
	  if (!scon) {
	    scon = hcon_allocslot (&env->storageunit_cache,series);    
	    hcon_init (scon, sizeof (DRMS_StorageUnit_t), DRMS_MAXHASHKEYLEN, 
	      (void (*)(const void *)) drms_su_freeunit, NULL);
	  ...
	  if (strcmp (su[i]->sudir, sudir)) {
		...
		"sudirs: '%s' != '%s'", series, sunum, su[i]->sudir, sudir);
	    return 1;
	  }
	}
#ifdef DEBUG
...
/*
 *  Create a new series in the database using the given record as template 
...
int drms_create_series (DRMS_Record_t *rec, int perms) {
  ...
  for (int i = 0; i < template->seriesinfo->pidx_num; i++) {
    template->seriesinfo->pidx_keywords[i] =
        drms_keyword_lookup (template,
	rec->seriesinfo->pidx_keywords[i]->info->name, 0);
  }
...
int drms_update_series (DRMS_Record_t *rec, int perms) {
  ...
  if (!template) {
    fprintf (stderr,
        "ERROR: Cannot update series '%s' because it does not exist\n", series);
    return 1;
  }
  drms_copy_record_struct (template, rec);  
  status = drms_insert_series (env->session, 1, template, perms);
			/*  If this is a client then tell the server that 
						a new series now exists. */     
  if (!status && !env->session->db_direct) {
    drms_send_commandcode (env->session->sockfd, DRMS_NEWSERIES);
    send_string (env->session->sockfd, series);
  }
...
int drms_create_series_fromprototype (DRMS_Record_t **prototype,
    const char *outSeriesName, int perms) {
  ...
  if (prototype) drms_destroy_recproto (prototype);
