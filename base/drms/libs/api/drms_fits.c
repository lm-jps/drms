/*
 *  drms_fits.c							2007.11.21
 *
 *  functions defined:
 *	drms_writefits
 *	drms_readfits
 *	drms_print_fitsheader
 */
//#define DEBUG
#define _GNU_SOURCE
...
int drms_writefits (const char* file, int compress, int headlen, char *header,
  ...
  if (compress) {
    ...
    nzdata = drms_compress (DRMS_COMP_RICE, arr->type, npix, arr->data, buflen,
        &zdata[len]);
    if (nzdata < 0) {
    ...
    CHECKSNPRINTF(snprintf (head, 2880, "%2d %2d %1d %8d %8d %8d\n", bitpix,
        64, 1, ndata_blocks, nzhead, nzdata), 2880);
  ...
  /*  The FITS standard stipulates storing binary data in big endian format  */
    ...
    drms_byteswap (arr->type, npix, arr->data);
			...
			/* File successfully written to disk. Close it and
...
DRMS_Array_t *drms_readfits (const char* file, int readraw, int *headlen,
  ...
  if (!(fh = fopen (file, "r"))) {
    ...
    if (*status) *status = DRMS_ERROR_IOERROR;
    return NULL;
  }
  ...
  if (compress == 0) {
	...
	goto bailout;
      ...
      for (i=0; i<36; ++i) {
	line = buf + i*80;
	if (!strncmp (line, "END     ", 8)) END_not_found = 0;
      }
      ++nhead_blocks;
    }  
    *headlen = nhead = nhead_blocks*2880;    
    ...
    if (fscanf (fh, "%d %d %d %d %d %d\n", &bitpix, &blk, &nhead_blocks,
        &ndata_blocks, &nzhead, &nzdata) != 6) {
    ...
    *headlen = nhead = nhead_blocks*2880;    
    ...
    if (rice_decode1 (zhead, nzhead, *header, nhead, 8)) {
      printkerr ("Decompressing FITS header failed with error code %d\n",
          status);
      free (zhead);
      free (*header);
      *header = NULL;
...
#endif
        out = drms_array_create (DRMS_TYPE_DOUBLE, arr->naxis, arr->axis, NULL,
	    NULL);
 ...
 *
 */
static int fits_get_sizes (int headlen, char *header, int *bitpix, int *naxis,
    int *axis) {
  int i;
  *naxis=0;
  *bitpix=0;
  ...
  if ( *naxis==0 || *bitpix==0 ) return 1;
				/*  Make sure we got all the axis lengths  */
  for  (i=0; i<*naxis; i++)
    if (axis[i] == 0) return 1;
