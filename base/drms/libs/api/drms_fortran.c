...
// ##################################
// ##### FILE:: drms_compress.c #####
// ####          START           ####
// ##################################

/* TODO REVISIT */
/* I think the unsigned char * should be understood as an
   array of bytes */
/*int f_drms_compress(DRMS_Compress_t method, DRMS_Type_t type, int nin, void *in, int maxout, unsigned char *out) {

  return drms_compress(method, type, nin, in, maxout, out);
}
FCALLSCFUN6(INT, f_drms_compress, F_DRMS_COMPRESS, f_drms_compress, INT, INT, INT, PVOID, INT, PSTRING)
*/
...
/* I think the unsigned char * should be understood as an
   array of bytes */
/*
int f_drms_uncompress(DRMS_Compress_t method, int nin, unsigned char *in, DRMS_Type_t type, int nout, void *out) {

  return drms_uncompress(method, nin, in, type, nout, out);
}
FCALLSCFUN6(INT, f_drms_uncompress, F_DRMS_UNCOMPRESS, f_drms_uncompress, INT, INT, PSTRING, INT, INT, PVOID)
*/
...
/* Modules should not use drms_writefits() or drms_readfits() directly.  They should
 * use drms_segment_write() and drms_segment_read().
 *
 * --Art Amezcua 8/13/2007 
 */

int f_drms_writefits(const char* file, int compress, int headlen, char *header, char * arr_hdl) {
  DRMS_Array_t  * arr = (DRMS_Array_t  *)  _convert_handle(arr_hdl);

  return drms_writefits(file, compress, headlen, header, arr);
}
FCALLSCFUN5(INT, f_drms_writefits, F_DRMS_WRITEFITS, f_drms_writefits, STRING, INT, INT, STRING, STRING)


// found a [char **header]
char * f_drms_readfits(const char* file, int readraw, int *headlen, void *header, int *status) {
  Fort_Alloc_t *header_alloc = (Fort_Alloc_t *) header;
  char *buffer=(char * ) NULL;    /* pointer to fits header */
  DRMS_Array_t  * _ret_var;
  _ret_var = drms_readfits(file, readraw, headlen, &buffer, status);

  if ( !*status ) {
    fort_alloc(header_alloc, buffer, *headlen/80, 80, 1);
  }

  return _pointer2handle((void *)_ret_var, "DRMS_Array_t", "drms_readfits");
}
FCALLSCFUN5(STRING, f_drms_readfits, F_DRMS_READFITS, f_drms_readfits, STRING, INT, PINT, PVOID, PINT)


char * f_drms_readfits2(const char* file, int readraw, int *headlen, char *header, int size, int *status) {
  DRMS_Array_t  * _ret_var;
  char *h;
  _ret_var = drms_readfits(file, readraw, headlen, &h, status);

  strcpy(header,h);
  free(h);
  return _pointer2handle((void *)_ret_var, "DRMS_Array_t", "drms_readfits2");
}
FCALLSCFUN5(STRING, f_drms_readfits2, F_DRMS_READFITS2, f_drms_readfits2, STRING, INT, PINT, PSTRING, PINT)

/*
void drms_print_fitsheader(int, char *)
*/
FCALLSCSUB2(drms_print_fitsheader, F_DRMS_PRINT_FITSHEADER, f_drms_print_fitsheader, INT, STRING)
...
int f_drms_tasfile_read(char *filename, DRMS_Type_t type, double bzero, double bscale, char * rf_hdl) {
  DRMS_Array_t  * rf = (DRMS_Array_t  *)  _convert_handle(rf_hdl);

  return drms_tasfile_read(filename, type, bzero, bscale, rf);
}
FCALLSCFUN5(INT, f_drms_tasfile_read, F_DRMS_TASFILE_READ, f_drms_tasfile_read, STRING, INT, DOUBLE, DOUBLE, STRING)


int f_drms_tasfile_readslice(char * fp_hdl, DRMS_Type_t type, double bzero, double bscale, int naxis, int *start, int *end, char * rf_hdl) {
  FILE  * fp = (FILE  *)  _convert_handle(fp_hdl);
  DRMS_Array_t  * rf = (DRMS_Array_t  *)  _convert_handle(rf_hdl);

  return drms_tasfile_readslice(fp, type, bzero, bscale, naxis, start, end, rf);
}
FCALLSCFUN8(INT, f_drms_tasfile_readslice, F_DRMS_TASFILE_READSLICE, f_drms_tasfile_readslice, STRING, INT, DOUBLE, DOUBLE, INT, PINT, PINT, STRING)


int f_drms_tasfile_writeslice(char * fp_hdl, double bzero, double bscale, int *start, char * array_hdl) {
  FILE  * fp = (FILE  *)  _convert_handle(fp_hdl);
  DRMS_Array_t  * array = (DRMS_Array_t  *)  _convert_handle(array_hdl);

  return drms_tasfile_writeslice(fp, bzero, bscale, start, array);
}
FCALLSCFUN5(INT, f_drms_tasfile_writeslice, F_DRMS_TASFILE_WRITESLICE, f_drms_tasfile_writeslice, STRING, DOUBLE, DOUBLE, PINT, STRING)


int f_drms_tasfile_write(char *filename, DRMS_Type_t type, double bzero, double bscale, DRMS_Compress_t compression, int *blocksize, char * array_hdl) {
  DRMS_Array_t  * array = (DRMS_Array_t  *)  _convert_handle(array_hdl);

  return drms_tasfile_write(filename, type, bzero, bscale, compression, blocksize, array);
}
FCALLSCFUN7(INT, f_drms_tasfile_write, F_DRMS_TASFILE_WRITE, f_drms_tasfile_write, STRING, INT, DOUBLE, DOUBLE, INT, PINT, STRING)


int f_drms_tasfile_create(char *filename, DRMS_Compress_t compression, DRMS_Type_t type, int naxis, int *axis, int *blocksize, char * header_hdl) {
  TAS_Header_t  * header = (TAS_Header_t  *)  _convert_handle(header_hdl);

  return drms_tasfile_create(filename, compression, type, naxis, axis, blocksize, header);
}
FCALLSCFUN7(INT, f_drms_tasfile_create, F_DRMS_TASFILE_CREATE, f_drms_tasfile_create, STRING, INT, INT, INT, PINT, PINT, STRING)


/*
int drms_tasfile_defrag(char *, char *)
*/
FCALLSCFUN2(INT, drms_tasfile_defrag, F_DRMS_TASFILE_DEFRAG, f_drms_tasfile_defrag, STRING, STRING)
