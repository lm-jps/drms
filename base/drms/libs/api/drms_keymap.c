 ...
 *	drms_keymap_term
 *	drms_keymap_extname
 *	drms_keymap_classidextname
 *	drms_keymap_classextname
 *	drms_keymap_intname
 *	drms_keymap_classidintname
 *	drms_keymap_classintname
 */
...
static int drms_keymap_initcltables () {
   ...
   if (!gClassTables)
     gClassTables = hcon_create (sizeof (DRMS_KeyMap_t), MAXCLKEY, KMFree,
         NULL, NULL, NULL, 0);
   ...
   if (!ret && gClassTables) hcon_destroy (&gClassTables);
 ...
 *
 */
const char *drms_keymap_classidextname (DRMS_KeyMapClass_t classid,
    const char *intName) {
  const char *ret = NULL;
 ...
 *
 */
const char *drms_keymap_classidintname (DRMS_KeyMapClass_t classid,
    const char *extName) {
  const char *ret = NULL;
					/*  First, get class string id  */
  const char *strid = KeyMapClassIDMap[classid];
