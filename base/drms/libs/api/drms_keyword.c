/*
 *  drms_keyword.c						2007.11.23
 *
 *  functions defined:
 *	drms_free_template_keyword_struct
 *	drms_free_keyword_struct
 *	drms_copy_keyword_struct
 *	drms_create_keyword_prototypes
 *	drms_keyword_print
 *	drms_keyword_printval
 *	drms_template_keywords
 *	drms_keyword_lookup
 *	drms_keyword_type
 *	drms_keyword_createinfocon
 *	drms_keyword_destroyinfocon
 *	drms_keyword_keysmatch
 *	drms_getkey_char
 *	drms_getkey_short
 *	drms_getkey_int
 *	drms_getkey_longlong
 *	drms_getkey_float
 *	drms_getkey_double
 *	drms_getkey_string
 *	drms_getkey
 ...
 *	drms_setkey_short
 *	drms_setkey_int
 *	drms_setkey_longlong
 *	drms_setkey_float
 *	drms_setkey_double
 *	drms_setkey_string
 *	drms_keyword_getintname
 *	drms_keyword_getintname_ext
 *	drms_keyword_getextname
 *	drms_keyword_getextname_ext
 *	drms_copykey
 */
//#define DEBUG 
#include "drms.h"
...
/*
 *   target must have no existing links, since this function fills in the
 *    link container
 */
HContainer_t *drms_create_keyword_prototypes (DRMS_Record_t *target,
   ...
   if (target != NULL && target->keywords.num_total == 0 && source != NULL) {
	 ...
	 if (sKey->info && strlen (sKey->info->name) > 0) {
	    XASSERT(tKey = hcon_allocslot_lower (&(target->keywords),
		sKey->info->name));
	    ...
	    if (tKey && tKey->info) {
		  ...
		  copy_string (&(tKey->value.string_val),
		      sKey->value.string_val);
	       } else {
      ...
      if (hit) hiter_destroy (&hit);
            if (*status == DRMS_SUCCESS) ret = &(target->keywords);
   } else {
...
/* 
 *  Build the keyword part of a dataset template by using
 *    the query result holding a list of 
...
int drms_template_keywords (DRMS_Record_t *template) {
  ...
  for (i = 0; i<(int)qres->num_rows; i++) {
    ...
    for (seg=0; seg<(per_segment==1?num_segments:1); seg++) {
      ...
      if (per_segment) sprintf (name, "%s_%03d", name0, seg);
      else strcpy (name, name0);
      ...
      db_binary_field_getstr (qres, i, 10, sizeof (key->info->description),
	...
	db_binary_field_getstr (qres, i, 6, sizeof (key->info->format),
	    key->info->format);
	db_binary_field_getstr (qres, i, 7, sizeof (key->info->unit),
	    key->info->unit);
	key->info->isconstant   = db_binary_field_getint (qres, i, 8);
	key->info->per_segment  = db_binary_field_getint (qres, i, 9);
      } else {
	db_binary_field_getstr (qres, i, 2, sizeof (key->info->linkname),
	    key->info->linkname);
	db_binary_field_getstr (qres, i, 3, sizeof (key->info->target_key),
	    key->info->target_key);
...
HContainer_t *drms_keyword_createinfocon (DRMS_Env_t *drmsEnv,
    const char *seriesName, int *status) {
     ...
     DRMS_Record_t *template = drms_template_record (drmsEnv, seriesName,
	status);
		 ...
		 size);
	       ...
	       if (nameArr != NULL && valArr != NULL) {
		    ...
		    ret = hcon_create (sizeof (DRMS_KeywordInfo_t),
		        DRMS_MAXNAMELEN, NULL, NULL, (void **)valArr, nameArr,
			size);
	       } else {
		    *status = DRMS_ERROR_OUTOFMEMORY;
	       }
...
int drms_setkey (DRMS_Record_t *rec, const char *key, DRMS_Type_t type,
  ...
  if (rec ->readonly) return DRMS_ERROR_RECORDREADONLY;

  keyword = drms_keyword_lookup (rec, key, 0);
  if (keyword != NULL ) {
    if (keyword->info->islink) return DRMS_ERROR_KEYWORDREADONLY;
    else {
      //      rec->key_dirty = 1;
      return drms_convert (keyword->info->type, &keyword->value, type, value);
    }
  } else return DRMS_ERROR_UNKNOWNKEYWORD;  
}  
/*
 *  This is a more generic version of drms_setkey() - it assumes that the
 *    source type is the same as the target type.
 */
int drms_setkey_p (DRMS_Record_t *rec, const char *key, DRMS_Value_t *value) {
  ...
  if (keyword != NULL ) {
    if (keyword->info->islink) return DRMS_ERROR_KEYWORDREADONLY;
      ...
      //      rec->key_dirty = 1;
      return drms_convert (keyword->info->type, &keyword->value, value->type,
          &(value->value));
    }
  } else return DRMS_ERROR_UNKNOWNKEYWORD;  
}  
...
int drms_setkey_char (DRMS_Record_t *rec, const char *key, char value) {
  ...
  if (keyword != NULL ) {
    if (keyword->info->islink) return DRMS_ERROR_KEYWORDREADONLY;
      ...
      //    rec->key_dirty = 1;
      val.char_val = value;
      return drms_convert (keyword->info->type, &keyword->value,
	  DRMS_TYPE_CHAR, &val);
    }
  } else return DRMS_ERROR_UNKNOWNKEYWORD;  
}
...
int drms_setkey_int (DRMS_Record_t *rec, const char *key, int value) {
  ...
  if (keyword != NULL ) {
    if (keyword->info->islink) return DRMS_ERROR_KEYWORDREADONLY;
      ...
      //    rec->key_dirty = 1;
      val.int_val = value;
      return drms_convert (keyword->info->type, &keyword->value, DRMS_TYPE_INT,
	  &val);
    }
  } else return DRMS_ERROR_UNKNOWNKEYWORD;  
}
...
int drms_setkey_string (DRMS_Record_t *rec, const char *key, const char *value) {
  ...
  if (rec->readonly) ret = DRMS_ERROR_RECORDREADONLY;
  ...
  if (keyword != NULL ) {
    if (keyword->info->islink)
      ...
      ret = drms_convert (keyword->info->type, &keyword->value,
	  DRMS_TYPE_STRING, &val);
      free (val.string_val);
      val.string_val = NULL;
    }
  } else ret = DRMS_ERROR_UNKNOWNKEYWORD;
  return ret;
}
/*
 *
...
int drms_keyword_getintname_ext (const char *keyname,
    DRMS_KeyMapClass_t *classid, DRMS_KeyMap_t *map, char *nameOut, int size) {
   int success = 0;
   ...
   *nameOut = '\0';
   ...
   if (!success) {
      ...
      if (success) strncpy (nameOut, buf, size);
   }
   return success;
}
/*
 ...
 *
 */
int drms_copykey (DRMS_Record_t *target, DRMS_Record_t *source,
    const char *key) {
  int status = DRMS_SUCCESS;
  DRMS_Value_t srcval;				/* owns contained string */

  srcval = drms_getkey_p (source, key, &status);
  if (status == DRMS_SUCCESS) status = drms_setkey_p (target, key, &srcval);
