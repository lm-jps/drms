/*
 *  drms_link.c						2007.11.23
 *
 *  functions defined:
 *	drms_free_template_link_struct
 *	drms_free_link_struct
 *	drms_copy_link_struct
 *	drms_create_link_prototypes
 *	drms_setlink_static
...
/*
 *  target must have no existing links, since this function fills in
 *    the link container
 */
HContainer_t *drms_create_link_prototypes (DRMS_Record_t *target,
   ...
   if (target != NULL && target->links.num_total == 0 && source != NULL) {
	 ...
	 if (sLink->info && strlen(sLink->info->name) > 0) {
	    XASSERT(tLink = hcon_allocslot_lower (&(target->links),
		sLink->info->name));
	    ...
	    if (tLink && tLink->info) {
							/*  record  */
	      tLink->record = target;
	      memcpy (tLink->pidx_value, sLink->pidx_value,
		  ...
		  tLink->info->pidx_name[idx] =
		      strdup (sLink->info->pidx_name[idx]);
	 ...
	 } else *status = DRMS_ERROR_INVALIDLINK;
      }
      
      if (hit) hiter_destroy (&hit);
      if (*status == DRMS_SUCCESS) ret = &(target->links);
...
int drms_setlink_static (DRMS_Record_t *rec, const char *linkname,
  ...
  if (link->info->type==STATIC_LINK) {    
    ...
    //    rec->link_dirty = 1;
    return DRMS_SUCCESS;
  } else return DRMS_ERROR_INVALIDLINKTYPE;
}
/*
 *
 */
...
DRMS_Record_t *drms_link_follow (DRMS_Record_t *rec, const char *linkname,
  ...
  if ( (link = hcon_lookup_lower (&rec->links,linkname)) == NULL ) {
    if (status) *status = DRMS_ERROR_UNKNOWNLINK;
    return NULL;
  }
  if ((link->info->type == STATIC_LINK && link->recnum==-1) ||
      (link->info->type == DYNAMIC_LINK && !link->isset)) {
    if (status) *status = DRMS_ERROR_LINKNOTSET;
    return NULL;
  }
  if (drms_link_resolve (link)) {
    if (status) *status = DRMS_ERROR_BADLINK;
    return NULL;
  }
  return drms_retrieve_record (rec->env, link->info->target_series,
      link->recnum, status);  
}
/*
...
DRMS_RecordSet_t *drms_link_followall (DRMS_Record_t *rec, const char *linkname,
  ...
  if (drms_link_resolveall (link, &result->n, &recnums)) {
    if (recnums) free (recnums);
    stat = DRMS_ERROR_BADLINK;
    goto bailout;
  }
  XASSERT(result->records = malloc (result->n*sizeof (DRMS_Record_t *)));
  for (i=0; i<result->n; i++) {
    result->records[i] = drms_retrieve_record (rec->env,
	link->info->target_series, recnums[i], &stat);  
    if (stat) {
...
/*
 *  Resolve dynamic links by selecting the record with highest record number
...
static int drms_link_resolve (DRMS_Link_t *link) {
  ...
  if (link->info->type == STATIC_LINK || link->recnum>=0 ) return 0;
  else {
    ...
    maxrecnum = recnums[0];
    for(i=1; i<n; i++) {
      if (recnums[i] > maxrecnum) maxrecnum = recnums[i];
    }
 ...
 */
static int drms_link_resolveall (DRMS_Link_t *link, int *n,
    long long **recnums) {
  ...
  if (link->info->type == STATIC_LINK ) {
    ...
    *p = 0;
    ...
    qres = drms_query_bin_array (env->session, query, link->info->pidx_num,
        intype, argin);
    if (qres==NULL || qres->num_rows <= 0) return 1;
		/*  Find the candidate with the highest record number. */
    *n = qres->num_rows;
    XASSERT(*recnums = malloc (*n*sizeof (long long)));
	...
	link->info->target_series, link->recnum);
#endif
...
int drms_template_links (DRMS_Record_t *template) {
  ...
  for (i = 0; i<(int)qres->num_rows; i++) {
    ...
    db_binary_field_getstr (qres, i, 1, DRMS_MAXNAMELEN,
	link->info->target_series);
    db_binary_field_getstr (qres, i, 2, DRMS_MAXNAMELEN, buf);
    if (!strcmp (buf, "static")) link->info->type = STATIC_LINK;
    else if (!strcmp (buf, "dynamic")) link->info->type = DYNAMIC_LINK;
    else {
      fprintf (stderr,"ERROR: '%s' is not a valid link type.\n", buf);
      goto bailout;
    }	
    db_binary_field_getstr (qres, i, 3, DRMS_MAXCOMMENTLEN,
 ...
 *    primary index keywords of the target series if it is a dynamic link
 */
