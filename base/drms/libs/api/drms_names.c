/*
 *  drms_names.c						2007.11.23
 *
 *  functions defined:
 *	parse_record_set
 *	sql_record_set
 *	free_record_set
 *.	drms_recordset_query
 */
// #define DEBUG
#include "drms.h"
...
RecordSet_t *parse_record_set (DRMS_Env_t *env, char **in) {
  ...
  query_start = *in;		/*  Handy for error messages. */
  recnum_filter = 0;
  ...
  if (!parse_name (&p, rs->seriesname, DRMS_MAXNAMELEN)) {
    ...
    if ((rs->template = drms_template_record(env,rs->seriesname,&status)) ==
	NULL) {
...
static RecordSet_Filter_t *parse_record_set_filter (DRMS_Record_t *template, 
 ...
 error:
  while (head) {rsp = head->next; free(head); head = rsp;}
 error1:
  return NULL;
}
/*
...
static PrimekeyRangeSet_t *parse_primekey_set (DRMS_Keyword_t *keyword,
  ...
  if (*p=='#') {    
    ...
    if ((pks->index_rangeset = parse_index_set(&p))==NULL && syntax_error)
      goto error;
  } else {
    pks->type = VALUE_RANGE;
    if ((pks->value_rangeset = parse_value_set( keyword->info->type,&p)) ==
        NULL && syntax_error) goto error;
  }
  *in=p;
...
static IndexRangeSet_t *parse_index_set (char **in) { 
  ...
  do {
    ...
    if (ir->type != FIRST_VALUE && ir->type != LAST_VALUE) {
      if (*p == '-') {
	ir->type = RANGE_END;
      } else {
	...
	if (*p == '-') ir->type = START_END;
	else if (*p == '/') ir->type = START_DURATION;
	else ir->type = SINGLE_VALUE;
      }
	  ...
	  if (end == p) {
		...
		++syntax_error;      
		goto error;
	      } else if (ir->type == RANGE_END) ir->type = RANGE_ALL;
	      else ir->type = RANGE_START;
	  } else p = end;    
 ...
 error:
  while (head) { ir = head->next; free(head); head = ir; }
  return NULL;
}
/*
 *
...
static ValueRangeSet_t *parse_value_set (DRMS_Type_t datatype, char **in) {
  ...
  do {
    ...
    if (vr->type != FIRST_VALUE && vr->type != LAST_VALUE) {
							/*  Get start  */
      ...
      if (*p == '-') vr->type = START_END;
      else if (*p == '/') vr->type = START_DURATION;
      else vr->type = SINGLE_VALUE;
 ...
 error:
  printf ("ERROR\n");
  while (head) { vr = head->next; free (head); head = vr; }
  return NULL;
}
/*
...
static int sql_record_set_filter (RecordSet_Filter_t *rs, DRMS_Env_t *env, 
				 char *seriesname, char **query);
static int sql_record_query (RecordQuery_t *rs, DRMS_Env_t *env, 
			    char **query);
static int sql_record_list (RecordList_t *rs, DRMS_Env_t *env, 
			   char *seriesname,  char **query);
static int sql_recnum_set (IndexRangeSet_t  *rs, DRMS_Env_t *env, 
			  char *seriesname, char **query);
static int sql_primekey_set (PrimekeyRangeSet_t *rs, DRMS_Env_t *env,
			    char *seriesname,char **query);
static int sql_primekey_index_set (IndexRangeSet_t *rs, DRMS_Keyword_t *keyword,
				  DRMS_Env_t *env, char *seriesname, char **query);
static int sql_primekey_value_set (ValueRangeSet_t *rs, DRMS_Keyword_t *keyword,
				  DRMS_Env_t *env, char *seriesname, char **query);
/*
 *
 */
int sql_record_set (RecordSet_t *rs, DRMS_Env_t *env, char *seriesname,
    char *query) {
  char *p = query;
						/*  char *field_list; */
#ifdef DEBUG
 ...
 */
  ...
  if (rs->recordset_spec) {
    /*    p += sprintf (p," WHERE "); */
    return sql_record_set_filter (rs->recordset_spec, env, seriesname, &p);  
  } else {
    *p = 0;
    return 0;
  }
...
static int sql_record_set_filter (RecordSet_Filter_t *rs, DRMS_Env_t *env, 
  ...
  do {
    ...
    switch (rs->type) {
    case RECORDQUERY:
      sql_record_query (rs->record_query, env, &p);
      break;
    case RECORDLIST:
      sql_record_list (rs->record_list, env, seriesname, &p);
      break;
    default:    
 ...
 *
 */
static int sql_record_query (RecordQuery_t *rs, DRMS_Env_t *env, char **query) {
  char *p = *query;
#ifdef DEBUG
...
static int sql_record_list (RecordList_t *rs, DRMS_Env_t *env,
  ...
  switch (rs->type) {
  case RECNUMSET:
    sql_recnum_set (rs->recnum_rangeset, env, seriesname, &p);
    break;
  case PRIMEKEYSET:
    sql_primekey_set (rs->primekey_rangeset, env, seriesname, &p);
    break;
  default:    
...
static int sql_recnum_set (IndexRangeSet_t  *rs, DRMS_Env_t *env, 
  ...
  do {
	...
	p += sprintf (p, "%lld<=recnum AND recnum<%lld ", rs->start,
	    rs->start+rs->x);
      if (rs->skip!=1) {
	if (rs->type == RANGE_END || rs->type == RANGE_ALL) 
	  p += sprintf (p, "AND (recnum-(select min(recnum) from %s))%%%lld=0 ",
	      seriesname, rs->skip);	  
	else
...
static int sql_primekey_set (PrimekeyRangeSet_t *rs,  DRMS_Env_t *env,
  ...
  if (rs->type == INDEX_RANGE) 
    return sql_primekey_index_set (rs->index_rangeset, rs->keyword, env,
        seriesname, query);
  else if (rs->type == VALUE_RANGE) 
    return sql_primekey_value_set(rs->value_rangeset, rs->keyword, env,
        seriesname, query);
  else 
    return 1;
}
...
static int sql_primekey_index_set (IndexRangeSet_t *rs, DRMS_Keyword_t *keyword,
  ...
  } else step = step_key->value;
  do {
    p += sprintf (p, "( ");
    if (rs->type == FIRST_VALUE) {
      p += sprintf (p, "%s=(select min(%s) from %s)", keyword->info->name,
	  keyword->info->name, seriesname);
    } else if (rs->type == LAST_VALUE) {
      p += sprintf (p, "%s=(select max(%s) from %s)", keyword->info->name,
	  keyword->info->name, seriesname);
    } else if (rs->type == SINGLE_VALUE) {
	...
	p += sprintf (p, ")+");
	p += drms_sprintfval (p, datatype, &base, 1);
	p += sprintf (p, ") AND %s<((%lld*", keyword->info->name,
	    rs->start+rs->x);
	p += drms_sprintfval (p, datatype, &step, 1);
	p += sprintf (p, ")+");
	p += drms_sprintfval (p, datatype, &base, 1);
	p += sprintf (p, ")");
      ...
      if (rs->skip!=1) {
	if (rs->type == RANGE_END || rs->type == RANGE_ALL) 
	  p += sprintf (p, " AND (cast (round((%s-(select min(%s) from %s))/",
	      keyword->info->name, keyword->info->name, seriesname);
	else 
	  p += sprintf (p, " AND (cast (round((%s-%lld)/",
	      keyword->info->name, rs->start);
	p += drms_sprintfval (p, datatype, &step, 1);
...
static int sql_primekey_value_set (ValueRangeSet_t *rs, DRMS_Keyword_t *keyword,
  ...
  do {
	...
	p += sprintf (p, "<=%s AND %s<=", keyword->info->name,
	    keyword->info->name);
...
void free_record_set (RecordSet_t *rs) {
  if (rs->recordset_spec) free_record_set_filter (rs->recordset_spec);    
  free (rs);
}
/*
 *
 ...
 *
 */
int drms_recordset_query (DRMS_Env_t *env, char *recordsetname, char **query,
    char **seriesname, int *filter, int *mixed) {
  RecordSet_t *rs;
  char *p = recordsetname;
  *mixed = 0;

  if ((rs = parse_record_set (env, &p))) {
    if (rs->recordset_spec && rs->recordset_spec->record_query) *mixed = 1;
    XASSERT(*query = malloc (DRMS_MAXQUERYLEN));
    *seriesname = strdup (rs->seriesname);
    *filter = !recnum_filter;
    sql_record_set (rs,env, *seriesname, *query);
    free_record_set (rs);
    return 0;
  }
  else return 1;
