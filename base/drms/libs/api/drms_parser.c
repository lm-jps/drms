/*
 *  drms_parser.c						2007.11.24
 *
 *  functions defined:
 *	drms_parse_description
 *	parse_keywords
 *	getkeyword
 */
// #define DEBUG
...
static int parse_segments (char *desc, DRMS_Record_t *template) {
  ...
  while (*start) {
    ...
    if (prefixmatch (p, "Data:")) {      
      if (parse_segment (&q, template, segnum)) return 1;
      ++segnum;
    }
    start += len+1;
    len = getnextline (&start);
...
static int parse_segment (char **in, DRMS_Record_t *template, int segnum) {
  ...
  if (!strcasecmp (scope, "link") ) {
    ...
    if (!gettoken (&q, seg->info->linkname, sizeof (seg->info->linkname)))
	goto failure;
    if (!gettoken (&q, seg->info->target_seg, sizeof (seg->info->target_seg)))
	goto failure;
								 /*  Naxis  */      
    ...
    if (getstring (&q, seg->info->description, sizeof (seg->info->description))
        < 0) goto failure;
  } else {
							/*  Simple segment  */
    seg->info->islink = 0;
...
static int parse_links (char *desc, DRMS_Record_t *template) {
  ...
  while (*start) {
    ...
    if (prefixmatch (p, "Link:")) {
      if (parse_link (&q, template)) return 1;
    }
    start += len+1;
    len = getnextline (&start);
  }
...
static int parse_link (char **in, DRMS_Record_t *template) {
  char *p,*q;
  char name[DRMS_MAXNAMELEN]={0}, target[DRMS_MAXNAMELEN]={0},
      type[DRMS_MAXNAMELEN]={0}, description[DRMS_MAXCOMMENTLEN]={0};
  DRMS_Link_t *link;
...
int parse_keywords (char *desc, DRMS_Record_t *template) {
  ...
  while (*start) {
    ...
    if (prefixmatch (p, "Keyword:")) {
      if (parse_keyword (&q, template)) return 1;
    }
    start += len+1;
    len = getnextline (&start);
  }
...
static int parse_keyword (char **in, DRMS_Record_t *template) {
  char *p,*q;
  char name[DRMS_MAXNAMELEN]={0}, type[DRMS_MAXNAMELEN]={0},
      linkname[DRMS_MAXNAMELEN]={0}, defval[DRMS_DEFVAL_MAXLEN]={0};
  char unit[DRMS_MAXUNITLEN]={0}, description[DRMS_MAXCOMMENTLEN]={0},
      format[DRMS_MAXFORMATLEN]={0};
  char target_key[DRMS_MAXNAMELEN]={0}, constant[DRMS_MAXNAMELEN]={0},
      scope[DRMS_MAXNAMELEN]={0}, name1[DRMS_MAXNAMELEN+10]={0};
  ...
  if (!strcasecmp (type, "link") ) {
    ...
    if (!strcasecmp (scope, "segment")) per_segment = 1;
    else if (!strcasecmp (scope, "record")) per_segment = 0;
    else goto failure;    
    for (seg=0; seg<(per_segment==1 ? num_segments :1); seg++) {    
	 ...
	 with the segment number formatted as "_%03d" appended to the name  */
      if (per_segment) sprintf (name1, "%s_%03d", name, seg);
...
static int parse_primaryindex (char *desc, DRMS_Record_t *template) {
  ...
  while (*start) {
	...
	while (p<=(start+len)  && isspace(*p)) ++p;
	q = name;
	while (p<=(start+len) && q<name+sizeof (name) && !isspace(*p) &&
	    *p != ',') *q++ = *p++;	       
	*q++ = 0;
	p++;
...
/*
 *  Skip empty lines and lines where the first non-whitespace character is '#'
 */
static int getnextline (char **start) {
  char *p;
  int length;
...
static int getstring (char **inn, char *out, int maxlen) {
  ...
  if (*in == 0) {
    fprintf (stderr,
	"%s, line %d: Expected non-empty string on line %d of JSOC series descriptor.\n",
	__FILE__, __LINE__, lineno);
    return -1;
  }
  ...
  if (*in == '"' || *in == '\'' ) {
    ...
    if ( *in==escape ) ++in;		 /*  advance past the closing quote. */
    //    printf ("len = %d\n", len);
  } else {
...
static int getfloat (char **in, float *val) {
  ...
  *val = strtof (*in, &endptr);
  if (*val==0 && endptr==*in ) {
      fprintf (stderr,
	  "%s, line %d: The string '%s' on line %d of JSOC series descriptor is not a float.\n",
	  __FILE__, __LINE__, *in, lineno);
      return 1;
  } else {
    *in = endptr;
...
static int gettoken (char **in, char *copy, int maxlen) {
  int len;
  if ((len = getstring (in, copy, maxlen))<=0) return 0;
  if (stripcomma (in, copy, len, maxlen)) return 0;
  else return len;
}
/*
 *  Really ugly hack
