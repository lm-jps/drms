/*
 *  drms_record.c						2007.11.24
 *
 *  functions defined:
 *	drms_open_localrecords
 *	drms_open_dsdsrecords
 *	drms_open_records
 *	drms_create_records
 *	drms_create_records_fromtemplate
 ...
 */
static int IsLocalSpec (const char *recSetSpecIn, DSDS_KeyList_t ***klarrout,
    DRMS_Segment_t **segarrout, int *nRecsout, char **pkeysout, int *status) {
  int isLocSpec = 0;
  struct stat stBuf;
  ...
  char *recSetSpec = strdup (recSetSpecIn);
		/*  There could be an optional clause at the end of the
							file/directory spec. */
 ...
 */
	...
	pDSDSFn_DSDS_read_fitsheader_t pFn_DSDS_read_fitsheader = 
	    (pDSDSFn_DSDS_read_fitsheader_t)DSDS_GetFPtr (ghDSDS,
	    kDSDS_DSDS_READ_FITSHEADER);
	pDSDSFn_DSDS_free_keylist_t pFn_DSDS_free_keylist = 
	    (pDSDSFn_DSDS_free_keylist_t)DSDS_GetFPtr (ghDSDS,
	    kDSDS_DSDS_FREE_KEYLIST);
	pDSDSFn_DSDS_steal_seginfo_t pFn_DSDS_steal_seginfo =
	    (pDSDSFn_DSDS_steal_seginfo_t)DSDS_GetFPtr (ghDSDS,
	    kDSDS_DSDS_STEAL_SEGINFO);
	pDSDSFn_DSDS_free_seg_t pFn_DSDS_free_seg = 
	    (pDSDSFn_DSDS_free_seg_t)DSDS_GetFPtr (ghDSDS,
	    kDSDS_DSDS_FREE_SEG);
	...
	if (pFn_DSDS_read_fitsheader && pFn_DSDS_free_keylist &&
	  ...
	  if (S_ISREG(stBuf.st_mode) || S_ISLNK(stBuf.st_mode)) {
	    (*pFn_DSDS_read_fitsheader)(recSetSpec, &kl, &seg, kLocalSegName,
		&dsdsStat);
	    ...
	    if (dsdsStat == kDSDS_Stat_Success) {
	      ...
	      *segarrout = (DRMS_Segment_t *)malloc (sizeof (DRMS_Segment_t));
	      ...
	      (*pFn_DSDS_steal_seginfo)(&((*segarrout)[0]), seg);
		     /*  For local data, add filepath for use with later calls
						to  drms_segment_read(). */
	      snprintf ((*segarrout)[0].filename, DRMS_MAXSEGFILENAME, "%s",
		recSetSpec);
	      (*pFn_DSDS_free_seg)(&seg); *nRecsout = 1;
	    } else fitshead = 0;
	  } else  {
	    ...
	    if ((nFiles = scandir (recSetSpec, &fileList, NULL, NULL)) > 0 && 
	      ...
	      *klarrout =
	          (DSDS_KeyList_t **)malloc (sizeof (DSDS_KeyList_t *) * nFiles);
	      *segarrout =
		  (DRMS_Segment_t *)malloc (sizeof (DRMS_Segment_t) * nFiles);
	      while (fileIndex < nFiles) {
	        struct dirent *entry = fileList[fileIndex];
	        if (entry != NULL) {
		  char *oneFile = entry->d_name;
		  char dirEntry[PATH_MAX] = {0};
		  snprintf (dirEntry, sizeof (dirEntry), "%s%s%s", recSetSpec,
		      recSetSpec[strlen (recSetSpec) - 1] == '/' ? "" : "/",
		      oneFile);
		  if (*dirEntry !=  '\0' && !stat (dirEntry, &stBuf)); {
		    if (S_ISREG(stBuf.st_mode) || S_ISLNK(stBuf.st_mode)) {
		      (*pFn_DSDS_read_fitsheader)(dirEntry, &kl, &seg,
		          kLocalSegName, &dsdsStat);
		      if (dsdsStat == kDSDS_Stat_Success) {
		        (*klarrout)[iRec] = kl;
		...
		/* For local data, add filepath for use with later calls to 
			    ...
			    DRMS_MAXSEGFILENAME, "%s", dirEntry);
  ...
  if (recSetSpec) free(recSetSpec);
  if (status) *status = lstat;
  return isLocSpec;
}
/*
 *
 ...
 */
static int CreateRecordProtoFromFitsAgg (DRMS_Env_t *env,
    DSDS_KeyList_t **keylistarr, DRMS_Segment_t *segarr, int nRecs,
    char **pkeyarr, int nPKeys, DRMS_KeyMapClass_t fitsclass,
    DRMS_Record_t **proto, DRMS_Segment_t **segout, int *status) {
   ...
   if (template && template->seriesinfo) {
      ...
      hcon_init (&template->keywords, sizeof (DRMS_Keyword_t), DRMS_MAXHASHKEYLEN, 
		...
		(void (*)(const void *, const void *)) drms_copy_keyword_struct);
						/*  Loop through DSDS records -
      put a superset of keywords in rec->keywords and  ensure segments match */
	 ...
	 if (kl && oneSeg) {
	    while (kl != NULL && ((sKey = kl->elem) != NULL)) {
	       ...
	       if (!drms_keyword_getintname_ext (sKey->info->name, &fitsclass,
	           NULL, drmsKeyName, sizeof (drmsKeyName))) {
		  *drmsKeyName = '\0';
		  ...
		  break;
	       ...
	       if (!hcon_lookup_lower (&(template->keywords), drmsKeyName)) {
						   /*  insert into template  */
		  XASSERT(tKey = 
			  hcon_allocslot_lower (&(template->keywords),
			  drmsKeyName));
		  memset (tKey, 0, sizeof (DRMS_Keyword_t));
		  XASSERT(tKey->info = malloc (sizeof (DRMS_KeywordInfo_t)));
		  memset (tKey->info, 0, sizeof (DRMS_KeywordInfo_t));
		  ...
		  if (tKey && tKey->info) {
		     ...
		     tKey->record = template;
							   /*  keyword info  */
		     memcpy (tKey->info, sKey->info, sizeof (DRMS_KeywordInfo_t));
		     snprintf (tKey->info->name,  DRMS_MAXNAMELEN, "%s",
		         drmsKeyName);
						/*  default value - missing  */
		     drms_missing (tKey->info->type, &(tKey->value));
		  } else stat = DRMS_ERROR_OUTOFMEMORY;
	    ...
	    if (stat == DRMS_SUCCESS) {
		  ...
		  stat = DRMS_ERROR_INVALIDDATA;
			...
			/*  Add a keyword that will serve as primary key -
	       not sure what to use here. What about an increasing integer?  */
      if (fitsclass == kKEYMAPCLASS_LOCAL) {
	 if (!pkeysSpecified) AddLocalPrimekey (template, &stat);
	 else {
	/*  pkeysSpecified - ensure they exist; if kLocalPrimekey is specified,
						   but doesn't exist, add it */
	    int iKey;
 ...
 */
static void AdjustRecordProtoSeriesInfo (DRMS_Env_t *env,
    DRMS_Record_t *proto, const char *seriesName) {
							/*  series info  */
   char *user = getenv ("USER");

   snprintf (proto->seriesinfo->seriesname, DRMS_MAXNAMELEN, "%s", seriesName);
 ...
 *
 */
static void AllocRecordProtoSeg (DRMS_Record_t *template, DRMS_Segment_t *seg,
    int *status) {
   int stat = DRMS_SUCCESS;
 ...
 *
 */
static void SetRecordProtoPKeys (DRMS_Record_t *template, char **pkeyarr,
    int nkeys, int *status) {
   int stat = DRMS_SUCCESS;
 ...
 *
 */
static DRMS_Record_t *CacheRecordProto (DRMS_Env_t *env, DRMS_Record_t *proto,
   ...
   if (hcon_member_lower (&(env->series_cache), seriesName)) {
      ...
      cached = (DRMS_Record_t *)hcon_allocslot_lower (&(env->series_cache),
          seriesName);
	 ...
	 cached->seriesinfo->pidx_keywords[i] = 
	   ...
	   proto->seriesinfo->pidx_keywords[i]->info->name, 0);
      }
 ...
 */
static DRMS_RecordSet_t *CreateRecordsFromDSDSKeylist (DRMS_Env_t *env,
    int nRecs, DRMS_Record_t *cached, DSDS_KeyList_t **klarr,
    DRMS_Segment_t *segarr, int setPrimeKey, DRMS_KeyMapClass_t fitsclass,
    int *status) {
   int stat = DRMS_SUCCESS;
   int iRec;
   DRMS_RecordSet_t *rset =
       (DRMS_RecordSet_t *)malloc (sizeof (DRMS_RecordSet_t));
   rset->n = nRecs;
  ...
  /*  populate the new records with information from keylistarr and segarr  */
	 ...
	 while (stat == DRMS_SUCCESS && kl && ((sKey = kl->elem) != NULL)) {
	    ...
	    stat = drms_setkey (rset->records[iRec], drmsKeyName,
	        sKey->info->type, &(sKey->value));
	    kl = kl->next;
	 }			/*  while  */
	...
	/*  set and increment primekey (if no prime key specified by user ) */
	    if (setPrimeKey) {
	       stat = drms_setkey_longlong (rset->records[iRec],
	           kLocalPrimekey, primekeyCnt);
	       primekeyCnt++;
	    }
			/*  enter segment file names (kKEYMAPCLASS_LOCAL)  */
	    if ((tSeg = 
		 hcon_lookup_lower (&(rset->records[iRec]->segments),
		 kLocalSegName)) != NULL) {
	       snprintf (tSeg->filename, DRMS_MAXSEGFILENAME, "%s",
	           sSeg->filename);
	    } else {
	       stat = DRMS_ERROR_INVALIDDATA;
	       break;
 ...
 */
static DRMS_RecordSet_t *OpenLocalRecords (DRMS_Env_t *env,
    DSDS_KeyList_t ***klarr, DRMS_Segment_t **segarr, int nRecs, char **pkeys,
    int *status) {
   ...
   if (stat == DRMS_SUCCESS && ghDSDS) {
	  ...
	  (pDSDSFn_DSDS_free_keylistarr_t)DSDS_GetFPtr (ghDSDS,
	  kDSDS_DSDS_FREE_KEYLISTARR);
	 ...
	 if (pkeys && *pkeys) {
	    ...
	    nPkeys = 0;
	    while (pkeyname != NULL) {
	       if (!drms_keyword_getintname_ext (pkeyname, &fitsclass, NULL,
	           drmsKeyName, sizeof (drmsKeyName))) {
		  *drmsKeyName = '\0';
		  stat = DRMS_ERROR_INVALIDDATA;
		  break;
	       }
	 ...
	 if (stat == DRMS_SUCCESS) {
	    CreateRecordProtoFromFitsAgg (env, *klarr, *segarr, nRecs, pkeyarr,
	        nPkeys, kKEYMAPCLASS_LOCAL, &proto, &seg, &stat);
	 }
	 ...
	 if (stat == DRMS_SUCCESS)
	    rset = CreateRecordsFromDSDSKeylist (env, nRecs, cached, *klarr,
	        *segarr, setPrimeKey, kKEYMAPCLASS_LOCAL, &stat);
	 ...
	 *segarr = NULL;
	 ...
	 for (nPkeys = 0; nPkeys < DRMS_MAXPRIMIDX; nPkeys++)
	   if (pkeyarr[nPkeys]) free (pkeyarr[nPkeys]);
      }
   } else {
      fprintf (stdout,
...
DRMS_RecordSet_t *drms_open_localrecords (DRMS_Env_t *env,
    const char *dsRecSet, int *status) {
 ...
 */
DRMS_RecordSet_t *drms_open_dsdsrecords (DRMS_Env_t *env, const char *dsRecSet,
    int *status) {
   DRMS_RecordSet_t *rset = NULL;
   int stat = DRMS_SUCCESS;
   ...
   if (stat == DRMS_SUCCESS && ghDSDS) {
	  ...
	  (pDSDSFn_DSDS_open_records_t)DSDS_GetFPtr (ghDSDS,
	  kDSDS_DSDS_OPEN_RECORDS);
      pDSDSFn_DSDS_free_keylistarr_t pFn_DSDS_free_keylistarr = 
	  (pDSDSFn_DSDS_free_keylistarr_t)DSDS_GetFPtr (ghDSDS,
	  kDSDS_DSDS_FREE_KEYLISTARR);
      pDSDSFn_DSDS_free_segarr_t pFn_DSDS_free_segarr = 
	  (pDSDSFn_DSDS_free_segarr_t)DSDS_GetFPtr (ghDSDS,
	  kDSDS_DSDS_FREE_SEGARR);

      if (pFn_DSDS_open_records && pFn_DSDS_free_keylistarr &&
          pFn_DSDS_free_segarr) {
	char seriesName[DRMS_MAXNAMELEN];
 ...
 */
	nRecs = (*pFn_DSDS_open_records) (dsRecSet, seriesName, &hparams,
	    &keylistarr, &segarr, &dsdsStat);
	if (dsdsStat == kDSDS_Stat_Success) {
		 /*  make record prototype from this morass of information  */
	   DRMS_Record_t *template = NULL;
	   DRMS_Segment_t *seg = NULL;
	   ...
	   CreateRecordProtoFromFitsAgg (env, keylistarr, segarr, nRecs, NULL,
	       0, kKEYMAPCLASS_DSDS, &template, &seg, &stat);
	   if (stat == DRMS_SUCCESS) {
						     /*  Adjust seriesinfo  */
	   ...
	   if (stat == DRMS_SUCCESS) {
	      ...
	      cached = CacheRecordProto (env, template, seriesName, &stat);
		       ...
		       /*  create a new record (read-only) for each record  */
	   rset = CreateRecordsFromDSDSKeylist(env, nRecs, cached, keylistarr,
	       segarr, 0, kKEYMAPCLASS_DSDS, &stat);
 ...
 */
  ...
  if (stat == DRMS_SUCCESS) {
	...
	if (oneSet && strlen (oneSet) > 0) {
	   if (IsLocalSpec (oneSet, &klarr, &segarr, &nRecsLocal, &pkeys,
	       &stat)) {
	     rs = OpenLocalRecords (env, &klarr, &segarr, nRecsLocal, &pkeys,
	         &stat);
		...
		goto failure;
	      ...
	      TIME(stat = drms_recordset_query (env, oneSet, &query,
	          &seriesname, &filter, &mixed));
	      if (stat) goto failure;
	      ...
	      TIME(rs = drms_retrieve_records (env, seriesname, query, filter,
	          mixed, &stat));
#ifdef DEBUG
	   ...
	   if (nsets == 1) {
	     ...
	     ret = rs;
	      ...
	      if (!realSets) {
		realSets = hcon_create (sizeof (DRMS_RecordSet_t *), 64,
	...
	if (nsets > 1) {
		 ...
		 for (iSet = 0; iSet < nsets; iSet++) {
		    ...
		    if ((prs = hcon_lookup (realSets, buf)) != NULL) {
			  ...
			  for (i = 0; i < oners->n; i++) {
		       ...
		       oners = NULL;
		    } else stat = DRMS_ERROR_INVALIDDATA;
		 }	/* iSet */
	      }
	   }
	}
     ...
     if (realSets) hcon_destroy (&realSets);
...
DRMS_RecordSet_t *drms_create_records_fromtemplate (DRMS_Env_t *env, int n,
    DRMS_Record_t *template, DRMS_RecLifetime_t lifetime, int *status) {
  int i=0, stat;
  DRMS_RecordSet_t *rs=NULL;
  ...
  if (n<1) {
    if (status) *status = DRMS_ERROR_BADRECORDCOUNT;
    return NULL;
  }
  ...
  for (i=0; i<n; i++) {
    if ((rs->records[i] = drms_alloc_record2 (template, recnum[i], &stat)) ==
        NULL) goto failure;    
  ...
  if (drms_record_numsegments (rs->records[0])) {
	...
	while ((seg = (DRMS_Segment_t *)hiter_getnext (&hit))) {
	  if (seg->info->protocol == DRMS_TAS) {
	    ...
	    drms_tasfile_create (filename, DRMS_COMP_RICE, seg->info->type,
	        seg->info->naxis+1, seg->axis, seg->blocksize, NULL);
    ...
    for (i=0; i<n; i++) rs->records[i]->su = NULL;
...
failure:
  if (rs) drms_close_records (rs, DRMS_FREE_RECORD);
  if (recnum) free (recnum);
  if (status) *status = stat;
  return NULL;
}
/*
...
DRMS_RecordSet_t *drms_create_recprotos (DRMS_RecordSet_t *recset, int *status) {
   ...
   if (detached && detached->records) *status = DRMS_SUCCESS;
...
void drms_destroy_recproto (DRMS_Record_t **proto) {
   if (proto) {
	 ...
	 if (rec->seriesinfo == prototype->seriesinfo) {
      ...
      if (deep) drms_free_template_record_struct (prototype);
      else drms_free_record_struct (prototype);
      free (prototype);
      *proto = NULL;
   }
}
  ...
  while (first < n_total) {
    ...
    for (i=0; i<n; i++) {
      rs_out->records[first+i] = drms_alloc_record2 (rs_in->records[first+i],
          recnum[i], &stat);
      if (rs_out->records[first+i] == NULL) goto failure;    
...
/*  Process nonlink data segments if this series has any associated with it. */ 
	...
	if ((stat = drms_newslots (env, n, series, recnum, lifetime, slotnum,
	    su))) goto failure;    
/*  Copy record directories and TAS data segments (slices) record-by-record. */
	for (i=0; i<n; i++) {     
	  ...
	  while ((seg_in = (DRMS_Segment_t *)hiter_getnext (&hit_in)) && 
		...
		seg_out->axis[seg_out->info->naxis] =
		    rec_out->seriesinfo->unitsize;
		seg_out->blocksize[seg_out->info->naxis] = 1; 
		drms_tasfile_create (filename, DRMS_COMP_RICE, seg_out->info->type,
		    seg_out->info->naxis+1, seg_out->axis, seg_out->blocksize,
		    NULL);
	...
	break;
      ...
      for (i=0; i<n; i++) rs_out->records[first+i]->su = NULL;
    first = last;
  }
...
int drms_close_records (DRMS_RecordSet_t *rs, int action) {
  ...
  case DRMS_FREE_RECORD:
    for (i=0; i<rs->n; i++) {
      rec = rs->records[i];
		/*  If this record was temporarily created by this session
					then free its storage unit slot. */
	...
	if ((status = drms_freeslot (rec->env, rec->seriesinfo->seriesname,
	  ...
	  fprintf (stderr,
	      "ERROR in drms_close_record: drms_freeslot failed with "
	      "error code %d\n", status);
	  break;
	}
      }
  ...
  case DRMS_INSERT_RECORD:
	...
	fprintf (stderr, "ERROR in drms_close_record: Could not insert record "
	    "in the database.\ndrms_insert_records failed with error code %d\n",
	    status);
      }
    }
    break;
...
void drms_free_records (DRMS_RecordSet_t *rs) {
  ...
  if (!rs) return;
  for (i=0; i<rs->n; i++) drms_free_record (rs->records[i]);
  if (rs->n>0 && rs->records) free (rs->records);
  rs->n = 0;
  free (rs);
}
 ...
 *    unit if no free slots exist. The keywords, links and data segments will
...
/*
 *  Remove a record from the record cache and free any memory allocated for it
 */
...
DRMS_RecordSet_t *drms_retrieve_records (DRMS_Env_t *env,
    const char *seriesname, char *where, int filter, int mixed, int *status) {
  int i,throttled;
  int stat = 0;
  long long recnum;
  DRMS_RecordSet_t *rs;
  ...
  if (filter && template->seriesinfo->pidx_num > 0) {
					  /*  query on the latest version  */
    if (mixed) {
				/*  query on both prime and non-prime keys */
      p += sprintf (p, "select %s from %s, (select q2.max1 as max from  (select max(recnum) as max1, min(q1.max) as max2 from %s, (select %s, max(recnum) from %s where %s group by %s) as q1 where %s.%s = q1.%s",
          field_list, series_lower, series_lower, pidx_names, series_lower, where,
	  pidx_names, series_lower,
	  template->seriesinfo->pidx_keywords[0]->info->name,
	  template->seriesinfo->pidx_keywords[0]->info->name);
      for (i = 1; i < template->seriesinfo->pidx_num; i++) {
	p += sprintf (p, " and %s.%s = q1.%s", series_lower,
	    template->seriesinfo->pidx_keywords[i]->info->name,
	    template->seriesinfo->pidx_keywords[i]->info->name);
      }
      p += sprintf (p, " group by %s) as q2 where max1 = max2) as q3 where %s.recnum = q3.max order by %s",
          pidx_names, series_lower, pidx_names);
    } else {
      // query only on prime keys
      p += sprintf (p, "select %s from %s where recnum in (select max(recnum) from %s where 1=1 ",
          field_list, series_lower, series_lower);
      if (where && *where) {
  ...
  throttled = (qres->num_rows == limit);
				  /*  Filter query result and initialize
				record data structures from template. */  
  XASSERT(rs = malloc (sizeof (DRMS_RecordSet_t)));
  if (qres->num_rows<1) {
    rs->n = 0;
    rs->records = NULL;    
  ...
  free (series_lower);
  if (status) {
    if (!throttled) *status = DRMS_SUCCESS;
    else {
...
bailout:
  free (series_lower);
  db_free_binary_result (qres);   
  for (i=0;i<rs->n;i++) drms_free_records (rs);
  free (rs);
bailout1:
  free (query);
  free (field_list);
...
*/
DRMS_Record_t *drms_alloc_record2 (DRMS_Record_t *template, long long recnum,
    int *status) {
  DRMS_Record_t *rec;
  char hashkey[DRMS_MAXHASHKEYLEN];
  char *series;
...
DRMS_Record_t *drms_template_record (DRMS_Env_t *env, const char *seriesname,
  ...
  if ((template = hcon_lookup_lower (&env->series_cache, seriesname)) == NULL) {
    if (status) *status = DRMS_ERROR_UNKNOWNSERIES;
    return NULL;
  }
 ...
 */
    ...
    if ((stat=drms_template_segments (template))) goto bailout;
    if ((stat=drms_template_links (template))) goto bailout;
    if ((stat=drms_template_keywords (template))) goto bailout;
	...
	*p++ = 0;
	...
	for (i=0; i<template->seriesinfo->pidx_num; i++)
      ...
      printf ("\n");    
#endif
    } else template->seriesinfo->pidx_num = 0;
    db_free_binary_result (qres);   
  }
  if (status) *status = DRMS_SUCCESS;
  return template;
...
void drms_free_template_record_struct (DRMS_Record_t *rec) {
  ...
  if (rec->init == 1 ) {	/* Don't try to free uninitialized templates. */
    (rec->links).deep_free =
        (void (*)(const void *)) drms_free_template_link_struct;
    hcon_free (&rec->links);

    (rec->segments).deep_free =
        (void (*)(const void *)) drms_free_template_segment_struct;
    hcon_free (&rec->segments);

    (rec->keywords).deep_free =
...
void drms_copy_record_struct (DRMS_Record_t *dst, DRMS_Record_t *src) {
  ...
  XASSERT(dst && src);
			/* Copy fields in the main structure and series info. */
  *dst = *src;
...
int drms_populate_records (DRMS_RecordSet_t *rs, DB_Binary_Result_t *qres) {
  ...
  for (row=0; row<qres->num_rows; row++) {
	...
	db_binary_field_getstr (qres, row, col++, DRMS_MAXSEGFILENAME,
	    seg->filename);
...
char *drms_field_list (DRMS_Record_t *rec, int *num_cols) {
  ...
  while ((link = (DRMS_Link_t *)hiter_getnext (&hit))) {
	...
	p += sprintf (p, ", ln_%s_isset", link->info->name);
		...
		/*  There is a field for each keyword in the primary index
				   of the target series...walk through them. */
      for (i=0; i<link->info->pidx_num; i++)
	p += sprintf (p, ", ln_%s_%s", link->info->name,
  ...
  buf[len] = 0;				    /* Hope we got the length right! */

  if (num_cols) *num_cols = ncol;
#ifdef DEBUG
...
int drms_insert_records (DRMS_RecordSet_t *recset) {
  ...
  if ((num_rows = recset->n) < 1) return 0;
...
/*  Now copy the actual values from the records into the argument arrays. */
  for (row=0; row<num_rows; row++) {
	...
	for (i=0; i<link->info->pidx_num; i++) {
	  if (link->info->pidx_type[i] == DRMS_TYPE_STRING ) {
	    memcpy (argin[col]+row*sz[col], &link->pidx_value[i].string_val,
	        sz[col]);
	    col++;
	  } else {
	    memcpy (argin[col]+row*sz[col],
	        drms_addr (link->info->pidx_type[i], &link->pidx_value[i]),
		sz[col]);
	    col++;
	  }
	}
	...
	if (key->info->type == DRMS_TYPE_STRING ) {
	  ...
	  memcpy (argin[col]+row*sz[col], drms_addr (key->info->type,
	      &key->value), sz[col]);
	  col++;
	}
      }    
  ...
  for (col=0; col<num_args; col++) free (argin[col]);
  free (argin);
  free (intype);
  free (query);
  free (sz);
...
int drms_record_numkeywords (DRMS_Record_t *rec) {
  if (rec) return hcon_size (&rec->keywords); 
  else return 0;
}
/*
 *
...
int drms_record_numsegments (DRMS_Record_t *rec) {
  if (rec) return hcon_size (&rec->segments); 
  else return 0;
}
/*
 *
...
FILE *drms_record_fopen (DRMS_Record_t *rec, char *filename, const char *mode) {
  ...
  case 'a':
    if (rec->su==NULL) {
      if ((stat = drms_newslots (rec->env, 1, rec->seriesinfo->seriesname,
          &rec->recnum, rec->lifetime, &rec->slotnum, &rec->su))) {
  ...
  if (!(fp = fopen (path, mode))) {
    ...
    if (newslot)		     /*  Ooops! Never mind about that slot!  */
      drms_freeslot (rec->env, rec->seriesinfo->seriesname, rec->su->sunum,
          rec->slotnum);
  }
  return fp;
}
...
long long drms_record_memsize (DRMS_Record_t *rec) {
  ...
  while ((key = (DRMS_Keyword_t *)hiter_getnext (&hit))) {
    ...
    if (!key->info->islink && !key->info->isconstant ) {
      if ( key->info->type == DRMS_TYPE_STRING ) {
	if (key->value.string_val) size += strlen (key->value.string_val); 
	else size += 40;
      }    
    }
  }
					   /*  Loop through Segment fields. */
  size += hcon_size (&rec->segments) *
      (sizeof (DRMS_Segment_t) + DRMS_MAXNAMELEN);
  return size;
}
/*
 *
...
static int CopyPrimaryIndex (DRMS_Record_t *target, DRMS_Record_t *source) {
   ...
   if (target != NULL && target->seriesinfo != NULL) {
	 ...
	 else status = DRMS_ERROR_INVALIDKEYWORD;
     }
 ...
 *
 */
int drms_recproto_setseriesinfo (DRMS_Record_t *rec, int *unitSize,
    int *bArchive, int *nDaysRetention, int *tapeGroup, const char *descr) {
   ...
   if (rec && rec->seriesinfo) {
	 ...
	 rec->seriesinfo->tapegroup = *tapeGroup;
      ...
      if (descr && strlen (descr) < DRMS_MAXCOMMENTLEN)
	 strcpy (rec->seriesinfo->description, descr);
...
char *drms_record_jsoc_version (DRMS_Env_t *env, DRMS_Record_t *rec) {
  ...
  snprintf (query, DRMS_MAXQUERYLEN,
      "select jsoc_version from %s.drms_session where sessionid = %lld",
