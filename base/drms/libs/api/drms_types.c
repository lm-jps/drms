...
/*
 *  Copy a field in a DB_Binary_Result_t table to a field in a
 *    DRMS_Type_Value_t union
 */
int drms_copy_db2drms (DRMS_Type_t drms_type, DRMS_Type_Value_t *drms_dst,
  ...
  case DRMS_TYPE_TIME: 
    ...
    break;
  case DRMS_TYPE_STRING: 
    if (drms_dst->string_val) free (drms_dst->string_val);
...
int drms_sprintfval_format (char *dst, DRMS_Type_t type, DRMS_Type_Value_t *val,
  ...
  if (format==NULL)
	...
	/*  From timeio.c: format should be a time zone or a standard time 
							like TAI/TDT/TT/UTC. */
...
int drms_sprintfval (char *dst, DRMS_Type_t type, DRMS_Type_Value_t *val,
  ...
  case DRMS_TYPE_TIME: 
    if (!internal) {
      sprint_time (dst, val->time_val, "UTC", 0);	   /* From timeio.c */
      return strlen (dst);
    } else
      return sprintf (dst, "%lf", val->time_val);
  case DRMS_TYPE_STRING: 
...
int drms2int (DRMS_Type_t type, DRMS_Type_Value_t *value, int *status) {
  ...
  case DRMS_TYPE_LONGLONG:   
    if (value->longlong_val == DRMS_MISSING_LONGLONG) {
      stat = DRMS_SUCCESS;
      result = DRMS_MISSING_INT;            
    } else if (!(value->longlong_val < INT_MIN || 
