   ...
   if (defined($dbh))
   {
       ...
       if ($rv == &kSuccess)
       {
           ...
           if ($#instsArr >= 0)
           {
               ...
               body => "This email message was automatically generated by the subscription system. '$series' has been unpublished.\nAs a result, all subscribers to this series have been unsubscribed from it. Should the series be published again,\nthe affected sites can re-subscribe to it. First each site needs to delete the series, then it needs to subscribe\nto it again. The sites can use the delete_series -k DRMS command to delete the series - the 'k' indicates that the\nexisting SUs should NOT be deleted. By retaining the SUs, the sites will not have to re-download them when they\nre-subscribe to the series.\n\nThe affected sites are:\n\n$instList\n",
               );
