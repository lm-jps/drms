    ...
    def run(self):
        try:
            while True: # request loop
                ...
                peerName = self.getID()
                self.log.writeDebug([ '[ RequestWorker::run() ] waiting for request from client ' + peerName ])
                        ...
                        if isinstance(req, DoneRequest):
                            ...
                            try:
                                ...
                                if textReceived == b'':
                                    ...
                                    # dump the miscellany on floor - try for 30 seconds
                                    timeStart = datetime.now()
                                    timeOutTime = timeStart + timedelta(seconds=30)
                                    toRead = [ self.sock ]
    ...
    def receiveMsg(self):
        ...
        while bytesReceivedTotal < RequestWorker.MSGLEN_NUMBYTES:
            ...
            if len(readable) > 0:
                ...
                self.log.writeDebug([ '[ receiveMsg()-1 ] ' + self.getID() + ' read ' + str(len(textReceived)) + ' bytes' ])
        ...
        while bytesReceivedTotal < numBytesMessage:
            ...
            if len(readable) > 0:
                ...
                self.log.writeDebug([ '[ receiveMsg()-2 ] ' + self.getID() + ' read ' + str(len(textReceived)) + ' bytes' ])
                allTextReceived.extend(textReceived)
                bytesReceivedTotal += len(textReceived)
            else:
                # a recv() would block
        ...
        try:
            while not self.sdEvent.is_set():
                ...
                if len(fdList) == 0:
                    ...
                    rw = RequestWorker(sock=clientSock, reqtable=self.reqtable, timeout=timedelta(seconds=30), log=self.log)
                    rw.start()
        finally:
