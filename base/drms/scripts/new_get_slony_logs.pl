...
my $config_file=$ARGV[2] or die ("No config_file specified");
...
$Net::SCP::scp="";                         		    	## path to fast scp instance.
my $rmt_hostname="";                      		        ## remote machine name
my $rmt_port="";                                       	## port to fast scp server in remote machine
...
while (<SETTINGS>) {
	...
	if ( $line[0] eq "Net_SCP_scp" ) {
		$Net::SCP::scp=$line[1];
	} elsif ( $line[0] eq "rmt_hostname" ) {
		$rmt_hostname=$line[1];
	} elsif ( $line[0] eq "rmt_port" ) {
		$rmt_port=$line[1];
