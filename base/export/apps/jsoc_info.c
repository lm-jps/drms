...
static int list_series_info(DRMS_Env_t *drms_env, DRMS_Record_t *rec, json_t *jroot, int followLinks, int useFitsKeyNames, int max_precision, int binary)
{
    int error = 0;
            ...
            if (!error)
            {
                if (drms_keyword_isindex(pkey))
                {
                    ...
                    if (!error)
                    {
                        ...
                        {
                           if (fitsexport_getmappedextkeyname(skey, NULL, NULL, fitsName, sizeof(fitsName), NULL, 0, NULL) && *fitsName != '\0')
                            {
                                keyNameOut = fitsName;
                            }
                            else
                    ...
                    if (!error)
                    {
                        ...
                        {
                            drms_keyword_snprintfval2(stepKey, rawval, sizeof(rawval), max_precision, binary);
                        }
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if (USE_FITS_NAMES_FOR_COLUMNS && useFitsKeyNames)
                    {
                        if (fitsexport_getmappedextkeyname(pkey, NULL, NULL, fitsName, sizeof(fitsName), NULL, 0, NULL) && *fitsName != '\0')
                        {
                            keyNameOut = fitsName;
                        }
                        else
        ...
        for (i=0; i<rec->seriesinfo->dbidx_num; i++)
        {
            ...
            else
            {
                if (!error)
                {
                    if (USE_FITS_NAMES_FOR_COLUMNS && useFitsKeyNames)
                    {
                        if (fitsexport_getmappedextkeyname(key, NULL, NULL, fitsName, sizeof(fitsName), NULL, 0, NULL) && *fitsName != '\0')
                        {
                            keyNameOut = fitsName;
                        }
                        else
        ...
        else
        {
            while ((key = drms_record_nextkey(jsdTemplate, &last, 0)))
            {
                ...
                if (key->info->islink)
                {
                    ...
                    if (!error)
                    {
                        if (fitsexport_getmappedextkeyname(linkedkw, NULL, NULL, fitsName, sizeof(fitsName), NULL, 0, NULL) && *fitsName != '\0')
                        {
                            keyNameOut = fitsName;
                        }
                        else
    ...
    if (from_web)
    {
        ...
        *webarglist = '\0';
        ...
        if (req)
        {
            ...
            if (SetWebArg(req, "B", &webarglist, &webarglistsz)) JSONDIE("Bad QUERY_STRING");
            if (SetWebArg(req, "l", &webarglist, &webarglistsz)) JSONDIE("Bad QUERY_STRING");
            if (SetWebArg(req, "M", &webarglist, &webarglistsz)) JSONDIE("Bad QUERY_STRING");
