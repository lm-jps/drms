...
int rice_encode1(
    const char *in,		// input array of pixels
...
{
    unsigned char *d;		// array of 1st differences to be encoded
    ...
    int i, j;
    signed char curr, last, delta;
    unsigned char top, bottom, kmask, tmp;
    double pixsum;

    d = (unsigned char *) malloc(blksz * sizeof(unsigned char));
    if (!d) return RICE_ENCODE_OUT_OF_MEMORY;

    memset(out, 0, bufsz);

    // output first pixel raw
    last = in[0];
    val = last;
    out[0] = val;
