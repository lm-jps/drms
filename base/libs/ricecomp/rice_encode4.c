...
int rice_encode4(
    const int *in,		// input array of pixels
...
{
    unsigned *d;		// array of 1st differences to be encoded
    unsigned char *c;		// pointer to current byte in output
    unsigned buf = 0;		// bit buffer
    int bits2go = 32;		// number of unfilled bits in bit buffer
    int k;			// number of low order bits to split
    int i, j, curr, last, delta;
    unsigned val, top, bottom, kmask, tmp;
    double pixsum;

    d = (unsigned *) malloc(blksz * sizeof(unsigned));
