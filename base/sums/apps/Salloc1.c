...
*/
/*
 * Handles sum_svc type calls to allocate storage. Registered at 
 * socket SUMALLOC1,SUMALLOCV. This is used the the SUM_alloc() API.
*/
  ...
  int sig;
{
  printf("sig = %d\n", sig); //!!TEMP
  if(sig == SIGTERM) {
    write_log("*** %s Salloc1 got SIGTERM. Exiting.\n", datestring());
    exit(1);
  }
  if(sig == SIGINT) {
    write_log("*** %s Salloc1 got SIGINT. Exiting.\n", datestring());
    DS_DisConnectDB();
    exit(1);
  }
  write_log("*** %s Salloc1 got an illegal signal %d, ignoring...\n",
			datestring(), sig);
...
{
  write_log("%s usr1_sig received by Salloc1\n", datestring());
  ...
  open_log(newlogname);
  write_log("\n## %s reopen log Salloc1 on %s for pid = %d ##\n",
...
void get_cmd(int argc, char *argv[])
{
  ...
  if(argc != 2) {
    ...
    printf("Salloc1 jsoc_sums sum_svc_2011.06.06.140140.log\n");
    exit(1);
  }
  dbname = argv[0];
...
int main(int argc, char *argv[])
{
  ...
  if(atexit(sumbye)) {
	...
	(void) pmap_unset(SUMALLOC1, SUMALLOCV);
	...
	if (!svc_register(transp, SUMALLOC1, SUMALLOCV, sumprog_1, IPPROTO_TCP)) {
		write_log("***unable to register (SUMALLOC1, SUMALLOCV, tcp)\n");
		exit(1);
	}
  ...
  if(!(simmode = (char *)getenv("SUMSIMMODE"))) {
      ...
      write_log("Salloc1 sets SUMPGPORT env to %s\n", pgport);
  }
  else {
      write_log("Salloc1 sim mode SUMPGPORT %s\n", (char *)getenv("SUMPGPORT"));
  }
...
//    exit(1);
//  }
//Salloc.c does below instead of SUM_Init()
