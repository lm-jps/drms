...
*/
/*
 * Handles sum_svc type calls to delete series. Registered at 
 * socket SUMDELSER,SUMDELSERV. This is used the the SUM_delete_series() API.
*/
  ...
  int sig;
{
  printf("sig = %d\n", sig); //!!TEMP
  if(sig == SIGTERM) {
    write_log("*** %s Sdelser got SIGTERM. Exiting.\n", datestring());
    exit(1);
  }
  if(sig == SIGINT) {
    write_log("*** %s Sdelser got SIGINT. Exiting.\n", datestring());
    DS_DisConnectDB();
    exit(1);
  }
  write_log("*** %s Sdelser got an illegal signal %d, ignoring...\n",
			datestring(), sig);
...
{
  write_log("%s usr1_sig received by Sdelser\n", datestring());
...
void get_cmd(int argc, char *argv[])
{
  ...
  if(argc != 2) {
    ...
    printf("Sdelser jsoc_sums sum_svc_2011.06.06.140140.log\n");
    exit(1);
  }
  dbname = argv[0];
...
int main(int argc, char *argv[])
{
  ...
  if(atexit(sumbye)) {
	...
	(void) pmap_unset(SUMDELSER, SUMDELSERV);
	...
	if (!svc_register(transp, SUMDELSER, SUMDELSERV, sumprog_1, IPPROTO_TCP)) {
	write_log("***unable to register (SUMDELSER, SUMDELSERV, tcp)\n");
	exit(1);
	}
  ...
  if(!(simmode = (char *)getenv("SUMSIMMODE"))) {
      ...
      write_log("Sdelser sets SUMPGPORT env to %s\n", pgport);
  }
  else {
      write_log("Sdelser sim mode SUMPGPORT %s\n", (char *)getenv("SUMPGPORT"));
  }
...
//    exit(1);
//  }
//Sdelser.c does below instead of SUM_Init()
