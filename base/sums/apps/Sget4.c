 ...
 * socket SUMGET4,SUMGETV. This is used the the SUM_get() API.
*/
  ...
  int sig;
{
  printf("sig = %d\n", sig); //!!TEMP
  if(sig == SIGTERM) {
    write_log("*** %s Sget4 got SIGTERM. Exiting.\n", datestring());
    exit(1);
  }
  if(sig == SIGINT) {
    write_log("*** %s Sget4 got SIGINT. Exiting.\n", datestring());
    DS_DisConnectDB();
    exit(1);
  }
  write_log("*** %s Sget4 got an illegal signal %d, ignoring...\n",
			datestring(), sig);
...
{
  write_log("%s usr1_sig received by Sget4\n", datestring());
  ...
  open_log(newlogname);
  write_log("\n## %s reopen log Sget4 on %s for pid = %d ##\n",
...
void get_cmd(int argc, char *argv[])
{
  ...
  if(argc != 2) {
    ...
    printf("Sget4 jsoc_sums sum_svc_2011.06.06.140140.log\n");
    exit(1);
  }
  dbname = argv[0];
...
int main(int argc, char *argv[])
{
  ...
  if(atexit(sumbye)) {
	...
	(void) pmap_unset(SUMGET4, SUMGETV);
	...
	if (!svc_register(transp, SUMGET4, SUMGETV, sumprog_1, IPPROTO_TCP)) {
		write_log("***unable to register (SUMGET4, SUMGETV, tcp)\n");
		exit(1);
	}
  ...
  if(!(simmode = (char *)getenv("SUMSIMMODE"))) {
      ...
      write_log("Sget4 sets SUMPGPORT env to %s\n", pgport);
  }
  else {
      write_log("Sget4 sim mode SUMPGPORT %s\n", (char *)getenv("SUMPGPORT"));
  }
  		...
  		result, (xdrproc_t)xdr_void, 0, TIMEOUT);
              if(clnt_stat != 0) {
                ...
                if(findkey(result, "uid")) {
                  ...
                  write_log("***Removing from Sget4 active list of clients\n");
