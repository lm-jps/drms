 ...
 * socket SUMINFO1,SUMINFOV. This is used the the SUM_infoEx() API.
*/
  ...
  int sig;
{
  printf("sig = %d\n", sig); //!!TEMP
  if(sig == SIGTERM) {
    write_log("*** %s Sinfo1 got SIGTERM. Exiting.\n", datestring());
    exit(1);
  }
  if(sig == SIGINT) {
    write_log("*** %s Sinfo1 got SIGINT. Exiting.\n", datestring());
    DS_DisConnectDB();
    exit(1);
  }
  write_log("*** %s Sinfo1 got an illegal signal %d, ignoring...\n",
			datestring(), sig);
...
{
  write_log("%s usr1_sig received by Sinfo1\n", datestring());
  ...
  open_log(newlogname);
  write_log("\n## %s reopen log Sinfo1 on %s for pid = %d ##\n",
...
void get_cmd(int argc, char *argv[])
{
  ...
  if(argc != 2) {
    ...
    printf("Sinfo1 jsoc_sums sum_svc_2011.06.06.140140.log\n");
    exit(1);
  }
  dbname = argv[0];
...
int main(int argc, char *argv[])
{
  ...
  if(atexit(sumbye)) {
	...
	(void) pmap_unset(SUMINFO1, SUMINFOV);
	...
	if (!svc_register(transp, SUMINFO1, SUMINFOV, sumprog_1, IPPROTO_TCP)) {
		write_log("***unable to register (SUMINFO1, SUMINFOV, tcp)\n");
		exit(1);
	}
  ...
  if(!(simmode = (char *)getenv("SUMSIMMODE"))) {
      ...
      write_log("Sinfo1 sets SUMPGPORT env to %s\n", pgport);
  }
  else {
      write_log("Sinfo1 sim mode SUMPGPORT %s\n", (char *)getenv("SUMPGPORT"));
  }
