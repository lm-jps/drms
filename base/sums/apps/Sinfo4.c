...
*/
/*
 * Handles sum_svc type calls to get ds info. Registered at 
 * socket SUMINFO4,SUMINFOV. This is used the the SUM_infoEx() API.
*/
  ...
  int sig;
{
  printf("sig = %d\n", sig); //!!TEMP
  if(sig == SIGTERM) {
    write_log("*** %s Sinfo4 got SIGTERM. Exiting.\n", datestring());
    exit(1);
  }
  if(sig == SIGINT) {
    write_log("*** %s Sinfo4 got SIGINT. Exiting.\n", datestring());
    DS_DisConnectDB();
    exit(1);
  }
  write_log("*** %s Sinfo4 got an illegal signal %d, ignoring...\n",
			datestring(), sig);
...
{
  write_log("%s usr1_sig received by Sinfo4\n", datestring());
  ...
  open_log(newlogname);
  write_log("\n## %s reopen log Sinfo4 on %s for pid = %d ##\n",
...
void get_cmd(int argc, char *argv[])
{
  ...
  if(argc != 2) {
    ...
    printf("Sinfo4 jsoc_sums sum_svc_2011.06.06.140140.log\n");
    exit(1);
  }
  dbname = argv[0];
...
void setup()
{
  ...
  //open_log(logname);	//moved into get_cmd()
  printk_set(write_log, write_log);
  write_log("\n## %s Sinfo4 on %s (%s) for pid = %d ##\n", 
...
int main(int argc, char *argv[])
{
  ...
  if(atexit(sumbye)) {
	...
	(void) pmap_unset(SUMINFO4, SUMINFOV);
	...
	if (!svc_register(transp, SUMINFO4, SUMINFOV, sumprog_1, IPPROTO_TCP)) {
		write_log("***unable to register (SUMINFO4, SUMINFOV, tcp)\n");
		exit(1);
	}
  ...
  if(!(simmode = (char *)getenv("SUMSIMMODE"))) {
      ...
      write_log("Sinfo4 sets SUMPGPORT env to %s\n", pgport);
  }
  else {
      write_log("Sinfo4 sim mode SUMPGPORT %s\n", (char *)getenv("SUMPGPORT"));
  }
...
//    exit(1);
//  }
//Sinfo.c does below instead of SUM_Init()
