 ...
 * socket SUMINFO5,SUMINFOV. This is used the the SUM_infoEx() API.
*/
  ...
  int sig;
{
  printf("sig = %d\n", sig); //!!TEMP
  if(sig == SIGTERM) {
    write_log("*** %s Sinfo5 got SIGTERM. Exiting.\n", datestring());
    exit(1);
  }
  if(sig == SIGINT) {
    write_log("*** %s Sinfo5 got SIGINT. Exiting.\n", datestring());
    DS_DisConnectDB();
    exit(1);
  }
  write_log("*** %s Sinfo5 got an illegal signal %d, ignoring...\n",
			datestring(), sig);
...
{
  write_log("%s usr1_sig received by Sinfo5\n", datestring());
  ...
  open_log(newlogname);
  write_log("\n## %s reopen log Sinfo5 on %s for pid = %d ##\n",
...
void get_cmd(int argc, char *argv[])
{
  ...
  if(argc != 2) {
    ...
    printf("Sinfo5 jsoc_sums sum_svc_2011.06.06.140140.log\n");
    exit(1);
  }
  dbname = argv[0];
...
void setup()
{
  ...
  //open_log(logname);	//now in get_cmd()
  printk_set(write_log, write_log);
  write_log("\n## %s Sinfo5 on %s (%s) for pid = %d ##\n", 
...
int main(int argc, char *argv[])
{
  ...
  if(atexit(sumbye)) {
	...
	(void) pmap_unset(SUMINFO5, SUMINFOV);
	...
	if (!svc_register(transp, SUMINFO5, SUMINFOV, sumprog_1, IPPROTO_TCP)) {
		write_log("***unable to register (SUMINFO5, SUMINFOV, tcp)\n");
		exit(1);
	}
