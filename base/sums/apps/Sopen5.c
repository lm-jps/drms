 ...
 * socket SUMOPEN5,SUMOPENV. This is used the the SUM_open() API.
*/
  ...
  int sig;
{
  printf("sig = %d\n", sig); //!!TEMP
  if(sig == SIGTERM) {
    write_log("*** %s Sopen5 got SIGTERM. Exiting.\n", datestring());
    exit(1);
  }
  if(sig == SIGINT) {
    write_log("*** %s Sopen5 got SIGINT. Exiting.\n", datestring());
    DS_DisConnectDB();
    exit(1);
  }
  write_log("*** %s Sopen5 got an illegal signal %d, ignoring...\n",
			datestring(), sig);
...
{
  write_log("%s usr1_sig received by Sopen5\n", datestring());
  ...
  open_log(newlogname);
  write_log("\n## %s reopen log Sopen5 on %s for pid = %d ##\n",
...
void get_cmd(int argc, char *argv[])
{
  ...
  if(argc != 2) {
    ...
    printf("Sopen5 jsoc_sums sum_svc_2011.06.06.140140.log\n");
    exit(1);
  }
  dbname = argv[0];
...
int main(int argc, char *argv[])
{
  ...
  if(atexit(sumbye)) {
	...
	(void) pmap_unset(SUMOPEN5, SUMOPENV);
	...
	if (!svc_register(transp, SUMOPEN5, SUMOPENV, sumprog_1, IPPROTO_TCP)) {
		write_log("***unable to register (SUMOPEN5, SUMOPENV, tcp)\n");
		exit(1);
	}
...
//    exit(1);
//  }
//Sopen5.c does below instead of SUM_Init()
