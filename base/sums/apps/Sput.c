 ...
 * socket SUMPUT,SUMPUTV. This is used the the SUM_put() API.
*/
...
{
  write_log("%s usr1_sig received by Sput\n", datestring());
  ...
  open_log(newlogname);
  write_log("\n## %s reopen log Sput on %s for pid = %d ##\n",
...
void get_cmd(int argc, char *argv[])
{
  ...
  if(argc != 2) {
    ...
    printf("Sput jsoc_sums sum_svc_2011.06.06.140140.log\n");
    exit(1);
  }
  dbname = argv[0];
...
int main(int argc, char *argv[])
{
  ...
  if(atexit(sumbye)) {
	...
	(void) pmap_unset(SUMPUT, SUMPUTV);
	...
	if (!svc_register(transp, SUMPUT, SUMPUTV, sumprog_1, IPPROTO_TCP)) {
		write_log("***unable to register (SUMPUT, SUMPUTV, tcp)\n");
		exit(1);
	}
  ...
  if(!(simmode = (char *)getenv("SUMSIMMODE"))) {
      ...
      write_log("Sput sets SUMPGPORT env to %s\n", pgport);
  }
  else {
      write_log("Sput sim mode SUMPGPORT %s\n", (char *)getenv("SUMPGPORT"));
  }
