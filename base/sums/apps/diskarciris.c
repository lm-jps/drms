...
int send_mail(char *fmt, ...)
{
  ...
  sprintf(cmd, "echo \"%s\" | Mail -s \"diskarc mail\" carl@sun.stanford.edu", string);
  system(cmd);
  va_end(args);
  return(0);
}
...
void usage()
{
  ...
  printf("Write the archive pending datasets to disk (or /dev/null)\n");
  printf("Usage:\n");
  printf(" diskarc [-v] [-d] [-q] -a archive_dir] database_name\n");
  printf(" where -v = Verbose mode\n");
  printf("       -d = Debug mode\n");
  printf("       -q = Query only to see what's available for archive\n");
  printf("       -a = Archive to disk instead of to tape.\n");
  printf("            Give the dir to cp the archived ds to.\n");
  printf("            Use /dev/null to not archive but make del pend.\n");
...
void get_cmd(int argc, char *argv[])
{
  ...
  if (adirflg) {
    adir = argv[0]; dbname = argv[1];
/*****************
    if(strstr(adir, "/dev/null")) {
      printf("WARNING: you are going archive the datasets to /dev/null.\n");
      printf("Ok to proceed? [y or n] = ");
      if(gets(line) == NULL) {
        printf("\nInvalid entry\n");
        exit(1);
      }
      if(strcmp(line, "y")) exit(0);
    }
******************/
  }
  ...
  //else dbname = argv[0];
  if(!adirflg) {
    printf("ERROR: Must speicfy either tape or disk write.\n\n");
    usage();
  }
}
...
//	       thishost, dbname,n);
 ...
 // }
  ...
  printf ("3-before connect to db where dbname:%s\n", dbname);
  if (DS_ConnectDB(dbname)) {
     fprintf(stderr,"Error in connecting to database %s\n", dbname);
     goaway();
  }
  is_connected=1;
  printf ("4-is connected to db where dbname:%s\n", dbname);
  call_tape_svc_cnt = 0;
}
...
{ 
  ...
  if(strcmp(username, "sumsadmin"  )) {
    printf("!!NOTE: You must be user sumsadmin to run diskarc!\n");
    exit(1);
  }
  ...
  printf ("0-Current effective_date is %ld\n", TODAY);
  time (&now); printf ("%s\n",asctime(localtime(&now)));
  printf ("1-Datasets will be archived if there are more than %d\n",
		archive_minimum);
  setup();
  ...
  if(test60d) {			//60day test mode. only archive groups 102-105
    ...
    //aplist = NC_PaRequest_AP(4);	//call groupset 4 -fix Carl to machine dcs2 code
    aplist = NC_PaRequest_AP(9);	//groupset 9 for iris
  }
  archive_pending = count_list(aplist);
  printf ("5-archive_pending count  %d\n", archive_pending);

  if (verbose || queryflg) {
    printf("\n6-There are %d entries in the archive pending list.\n",
         archive_pending);
    print_list ("7-archive pending list", aplist);
...
void do_disk_write( PADATA *p)
{
   int dosafe, permanent, archsub, apstatus;
   char cmd[256], pcmd[256], arch_tape_date[32];
   PADATA *walker;
   char subdir[50];
   char slotdir[50];

       strcpy(subdir,"iris");
       sprintf(slotdir,"%s%lu","D",p->ds_index);
       strcat(slotdir,"\0");

       /* first make directory */
       sprintf(cmd, "%s/%s", adir,subdir);
       if (!file_exists(cmd)) 
       {
         sprintf(cmd, "mkdir  %s/%s", adir,subdir);
         if(system(cmd)) {
           printf("!!FAILED: %s\n", cmd);
           return;
         }
   ...
   while(p) {
     if(strstr(adir, "/dev/null")) {
          printf("Effective cp of %s to /dev/null\n", p->wd);
     }
     else {
       printf("\nGROUPID=%d\n",p->group_id);
       /* first make directory */
       sprintf(cmd, "%s/%s", adir,subdir,slotdir);
       if (!file_exists(cmd)) 
       {
         sprintf(cmd, "mkdir  %s/%s/%s", adir,subdir,slotdir);
         if(system(cmd)) {
           printf("!!FAILED: %s\n", cmd);
           p = p->next;
           continue;
       ...
       /* Now cp over files to directory */
       //sprintf(cmd, "cp -rp %s %s", p->wd,adir);
       sprintf(cmd, "cp -rp %s  %s/%s", p->wd,adir,subdir);
       printf("%s\n", cmd);
       if(system(cmd)) {
         printf("!!FAILED: %s\n", cmd);
         p = p->next;
     ...
     sprintf(arch_tape_date, "%s", get_datetime());
     sprintf(cmd, "UPDATE SUM_MAIN SET ARCHIVE_STATUS = 'Y', ARCH_TAPE = '%s', ARCH_TAPE_DATE = '%s' WHERE DS_INDEX = %lu",
       adir, arch_tape_date, p->ds_index); 

    sprintf(pcmd, "echo \"%s\" | psql -p 5434 irisdb_sums", cmd);
    printf("%s\n", pcmd);
    if(system(pcmd)) {
      printf("Failed: %s\n", pcmd);
      exit(1);
    ...
    sprintf(cmd, "update sum_partn_alloc set status=%d where ds_index=%lu", 
                 apstatus, p->ds_index);
    sprintf(pcmd, "echo \"%s\" | psql -p 5434 irisdb_sums", cmd);
    printf("%s\n", pcmd);
    if(system(pcmd)) {
