...
#define NEWLOG_FILE "/home/production/cvs/JSOC/base/sums/apps/data/sum_rm_1.newlog"
#define RM_ASSIGN "/usr/local/logs/SUM/sum_rm_assign.txt"

int SUMN = 1;           //this is sum_rm_1
int stat_storage();
void get_cfg();
...
void alrm_sig(int sig)
{
  ...
  if((tstfp=fopen(NEWLOG_FILE, "r"))) { //close current log & start new one
    ...
    fclose(logfp);
    logvers++;
    sprintf(logname, "%s_1.%s.%d", xlogfile, timetag, logvers);
    open_log(logname);
    write_log("sum_rm_1: Start new log file %d\n", logvers);
    sprintf(cmd, "/bin/rm %s", NEWLOG_FILE);
    if(system(cmd)) {
      write_log("sum_rm_1: Can't execute %s.\n", cmd);
    }
  }
