...
{
  write_log("%s usr1_sig received by sum_rm_2\n", datestring());
  write_log("sum_rm_2 will exit at its next opportunity\n");
  usr1_sig_flag = 1;
  if(!active) {
      write_log("%s sum_rm_2 exit from usr1_sig\n", datestring());
      exit(0);
  }
}
...
void get_cfg()
{
  ...
  strcpy(xlogfile, "/tmp/sum_rm_2.log");
