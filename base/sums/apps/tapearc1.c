/*-----------------------------------------------------------------------------
 * /home/jim/cvs/JSOC/base/sum/tapearc1.c
 *
 * Like tapearc.c but calls NC_PaRequest_AP() for groupset 1
 *
  ...
  while(--argc > 0 && (*++argv)[0] == '-') {
    while((c = *++argv[0]))
      ...
      case '6':
        ...
        fprintf(stderr, "No test 60day mode for tapearc1\n");
        exit(1);
        break;
      default:
        usage();
...
***************************************/
  ...
  if (signal(SIGTERM, SIG_IGN) != SIG_IGN)
    ...
    (void) pmap_unset(TAPEARCPROG, TAPEARCVERS1);
    transp = svctcp_create(RPC_ANYSOCK, 0, 0);
    if (transp == NULL) {
            printf("***cannot create tcp service\n");
            exit(1);
    }
    if (!svc_register(transp, TAPEARCPROG, TAPEARCVERS1, tapearcprog_1, IPPROTO_TCP)) {
            printf("***unable to register (TAPEARCPROG, TAPEARCVERS1, tcp)\n");
            exit(1);
    }
...
int call_tape_svc(int groupid, double bytes, uint64_t index) {
  ...
  char *call_err;
    ...
    setkey_int(&alist, "tapearcvers", TAPEARCVERS1);
...
{ 
  ...
  if(test60d) {			//60day test mode. only archive groups 102-105
    ...
    aplist = NC_PaRequest_AP(1);	//use groupset 1
  }
