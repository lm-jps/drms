/*-----------------------------------------------------------------------------
 * /home/jim/cvs/JSOC/base/sum/tapearcinfo.c
 *
 * NOTE: This is the version that does query mode only and no db tables
 * are modified nor tape written.
 *
...
//!!TEMP reform walker to be by group_id
  ...
  walker = ap;
  printf("walker list s/b by group_id\n");
  while(walker) {
    printf("group_id=%d bytes=%15.6e\n", walker->group_id, walker->bytes);
    walker = walker->next;
  }
  walker = ap;
...
int storeunitarch(int docnt)
{
  ...
  while(walker) {
    ...
    if(i != 0) {	/* make sure didn't hit new group at beginning */
      ...
      **********************************************************************/
      printf("Call tape_svc here for group_id=%d bytes=%15.6e 1st ds_index=%lu\n",
		curr_group_id, total_bytes, first_index);
    }
    freekeylist(&alist);
    if(chunkcnt == docnt) {
      return(0);
