...
#where this value is 24 hours previous to the current time.
sub labeldate {
...
$TLMDS = "hmi.tlm_60d"; #series of .tlm files to query
$DB = "jsoc_sums";
$PGPORT = 5434;
$parcfile = "/tmp/test.parc";
$hrsprev = 24;
while ($ARGV[0] =~ /^-/) {
  $_ = shift;
  if (/^-h(.*)/) {
...
#First connect to database
  ...
  if ( !defined $dbh ) {
    ...
    $sql = "select online_loc, arch_tape, arch_tape_fn, arch_tape_date from sum_main where Arch_Tape_Date >= '$ldate' and Owning_Series = '$TLMDS'";
    $sth = $dbh->prepare($sql);
    if ( !defined $sth ) {
    ...
    open(PARC, ">$parcfile") || die "Can't open $parcfile: $!\n";
...
#      $file = `/bin/ls $dir/S00000/*.tlm`;
#      $pos = index($file, "VC0");
#      $ftlm = substr($file, $pos);
#      $pos = rindex($ftlm, '.');
#      $fftlm = substr($ftlm, 0, $pos);
