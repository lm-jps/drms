...
$abortfile = "/usr/local/logs/tapearc/TAPEARC_ABORT2";
$DB = "jsoc_sums";
...
while(1) {			#keep it running for now
  $label = &labeldate;
  $logfile = "/usr/local/logs/tapearc/tape_do_2_".$label;
  $cmd = "/home/production/cvs/JSOC/bin/linux_ia64/tapearc2 -v jsoc_sums";
  $sleeptime = 60;
  print "$cmd\n";
