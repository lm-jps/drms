...
/* Modified by Art Amezcua on 08/09/2007 - To every C function called by
 *   a Fortran function, every PSTRING parameter is followed by an int
 *   parameter that contains the length of the string.
 */
...
#else
/* This is the only branch relevant to JSOC 
 *   For the PSTRING type only- I added the length of the char buffer in a parameter 
 *   that follows the PSTRING.
 * 
 * --Art Amezcua 08/09/2007 */
#define         STRING_cfT(M,I,A,B,D)  TTTTSTR( A,B,D)
#define        STRINGV_cfT(M,I,A,B,D)  TTTTSTRV(A,B,D, num_elem(A,D,_3(M,_STRV_A,I)))
#define        PSTRING_cfT(M,I,A,B,D)    TTSTR( A,B,D), D+1
