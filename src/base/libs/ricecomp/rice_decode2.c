...
int rice_decode2(
    ...
    short *out,			// decoded 16-bit integer output array
    ...
    int blksz)			// number of pixels in a coding block
{
    ...
    int k, i, imax, m;
    unsigned short lastpix, diff, fs;
	...
	// high entropy case
	    ...
	    while (i < imax) {
		m = 16 - bits2go;	// need m more bits (9 <= m <= 16)
		diff = buf << m;
		m -= 8;
		diff += (unsigned short) (*c++) << m;
		buf = *c++;
		diff += buf >> bits2go;
		buf &= MASK;
